# Input Variables Analysis Report

Generated on: 2025-09-02T17:29:08.128Z

## Summary

- **Total template files analyzed**: 365
- **Files with input variables**: 365
- **Unique input variables found**: 819
- **Total variable occurrences**: 1746

## Variable Usage Frequency

Variables are listed in order of frequency of use across all templates.

### 1. ORGANIZATION_NAME
- **Occurrences**: 353
- **Used in**: 353 templates

*(Too many files to list - used in 353 templates)*

### 2. TARGET_AUDIENCE
- **Occurrences**: 89
- **Used in**: 89 templates

*(Too many files to list - used in 89 templates)*

### 3. PROGRAM_NAME
- **Occurrences**: 83
- **Used in**: 83 templates

*(Too many files to list - used in 83 templates)*

### 4. MISSION_STATEMENT
- **Occurrences**: 65
- **Used in**: 65 templates

*(Too many files to list - used in 65 templates)*

### 5. EVENT_NAME
- **Occurrences**: 20
- **Used in**: 20 templates

*(Too many files to list - used in 20 templates)*

### 6. TARGET_POPULATION
- **Occurrences**: 16
- **Used in**: 16 templates

*(Too many files to list - used in 16 templates)*

### 7. DOCUMENT_TYPE
- **Occurrences**: 12
- **Used in**: 12 templates

*(Too many files to list - used in 12 templates)*

### 8. TIME_PERIOD
- **Occurrences**: 11
- **Used in**: 11 templates

*(Too many files to list - used in 11 templates)*

### 9. DONOR_NAME
- **Occurrences**: 10
- **Used in**: 10 templates

### 10. EVENT_DATE
- **Occurrences**: 9
- **Used in**: 9 templates


### 11. TOPIC
- **Occurrences**: 9
- **Used in**: 9 templates

### 12. NUMBER
- **Occurrences**: 8
- **Used in**: 8 templates

### 13. EVENT_TYPE
- **Occurrences**: 8
- **Used in**: 8 templates

### 14. CONTENT_TYPE
- **Occurrences**: 8
- **Used in**: 8 templates

### 15. DATE
- **Occurrences**: 7
- **Used in**: 7 templates

### 16. LOCATION
- **Occurrences**: 7
- **Used in**: 7 templates

### 17. DONATION_AMOUNT
- **Occurrences**: 7
- **Used in**: 7 templates

### 18. MISSION_FOCUS
- **Occurrences**: 7
- **Used in**: 7 templates

### 19. TIMEFRAME
- **Occurrences**: 6
- **Used in**: 6 templates


### 20. PHONE_NUMBER
- **Occurrences**: 6
- **Used in**: 6 templates


### 21. TARGET_LANGUAGE
- **Occurrences**: 6
- **Used in**: 6 templates



### 22. FUNDER_NAME
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_low.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md
- Create_and_Communicate/Structured_Report_Creation/Grant_reports_and_compliance_documentation_low.md
- Create_and_Communicate/Structured_Report_Creation/Grant_reports_and_compliance_documentation_medium.md

### 23. EVENT_LOCATION
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_low.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_medium.md

### 24. BRACKETS
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_high.md
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_high.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_high.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Content_quality_assurance_and_editing_high.md

### 25. WORD_COUNT
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_medium.md
- Create_and_Communicate/Content_Generation_from_Scratch/Newsletter_content_and_email_campaigns_medium.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md
- Create_and_Communicate/Visual_Content_Creation/Social_media_graphics_and_quote_cards_low.md

### 26. POLICY_AREA
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Staff_handbooks_and_policy_manuals_low.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Staff_handbooks_and_policy_manuals_medium.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Policy_trend_analysis_and_implications_high.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Policy_trend_analysis_and_implications_low.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Policy_trend_analysis_and_implications_medium.md

### 27. KEY_MESSAGE
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_low.md
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_low.md
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_medium.md
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_low.md
- Create_and_Communicate/Visual_Content_Creation/Social_media_graphics_and_quote_cards_low.md

### 28. CRITERIA_LIST
- **Occurrences**: 5
- **Used in**: 5 templates

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Board_member_candidate_assessment_low.md
- Sort_and_Scan/Application_&_Candidate_Screening/Board_member_candidate_assessment_medium.md
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_medium.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Event_planning_proposal_scoring_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Training_material_quality_assessment_low.md

### 29. CONTACT_INFORMATION
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Plain_language_versions_of_complex_documents_high.md

### 30. EVENT_PURPOSE
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_low.md

### 31. STRATEGIC_PRIORITIES
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_high.md
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Media_coverage_tracking_and_analysis_high.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 32. BRACKETED_VARIABLES
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_high.md
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 33. LINK
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Newsletter_content_and_email_campaigns_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 34. DATA_SOURCE
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_low.md
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_low.md
- Create_and_Communicate/Structured_Report_Creation/Audit_responses_and_evaluation_reports_medium.md
- Learn_and_Decide/Data_Analysis_&_Insights/Financial_performance_analysis_low.md

### 35. CAUSE_AREA
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_high.md
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_medium.md
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_medium.md
- Create_and_Communicate/Content_Generation_from_Scratch/Social_media_posts_and_content_calendars_medium.md

### 36. PLATFORM_NAME
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_medium.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_high.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_low.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_medium.md

### 37. SECTOR_TYPE
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Creating_executive_summaries_from_detailed_reports_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Press_releases_and_media_announcements_medium.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Service_delivery_quality_evaluation_high.md

### 38. DESIRED_ACTION
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_low.md
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_medium.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md
- Create_and_Communicate/Visual_Content_Creation/Infographics_about_programs_and_impact_low.md

### 39. ORGANIZATION_MISSION
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Volunteer_agreements_and_contracts_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Donor_prospect_identification_and_ranking_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Feedback_and_complaint_categorization_low.md

### 40. LEARNING_OBJECTIVES
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Survey_design_and_evaluation_forms_medium.md
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_low.md
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Training_material_quality_assessment_medium.md

### 41. PROGRAM_TYPE
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Volunteer_engagement_and_retention_analysis_low.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_high.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_low.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_medium.md

### 42. EVALUATION_CRITERIA
- **Occurrences**: 4
- **Used in**: 4 templates

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_low.md
- Sort_and_Scan/Application_&_Candidate_Screening/Vendor_proposal_evaluation_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_medium.md

### 43. STAFF_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_high.md

### 44. CORE_VALUES
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 45. EMAIL
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_high.md

### 46. VOLUNTEER_COORDINATOR_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 47. COORDINATOR_EMAIL
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Automate_the_Admin/Automated_Communications/Volunteer_shift_reminders_and_updates_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 48. COORDINATOR_PHONE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Automate_the_Admin/Automated_Communications/Volunteer_shift_reminders_and_updates_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 49. CONTACT_EMAIL
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 50. CONTACT_PHONE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 51. SERVICE_TYPE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_medium.md

### 52. SERVICE_AREA
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md
- Automate_the_Admin/Form_&_Document_Review/Client_intake_form_validation_high.md
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_brochures_to_website_copy_high.md

### 53. STAFF_SIZE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_high.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_high.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 54. CLIENT_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_low.md

### 55. RECIPIENT_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_low.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_low.md

### 56. MEETING_DATE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_low.md
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 57. AMOUNT
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Social_media_posts_and_content_calendars_high.md

### 58. EXECUTIVE_DIRECTOR_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 59. SOURCE_SYSTEM
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_high.md
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_low.md
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_medium.md

### 60. CRM_SYSTEM
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_high.md
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_medium.md
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_high.md

### 61. REPORTING_PERIOD
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_high.md
- Create_and_Communicate/Structured_Report_Creation/Program_summaries_for_stakeholders_high.md

### 62. STAKEHOLDER_TYPE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md
- Create_and_Communicate/Structured_Report_Creation/Program_summaries_for_stakeholders_low.md
- Create_and_Communicate/Structured_Report_Creation/Program_summaries_for_stakeholders_medium.md

### 63. LENGTH
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_low.md
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_low.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_low.md

### 64. REPORT_TYPE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Creating_executive_summaries_from_detailed_reports_medium.md
- Create_and_Communicate/Structured_Report_Creation/Audit_responses_and_evaluation_reports_medium.md
- Create_and_Communicate/Structured_Report_Creation/Grant_reports_and_compliance_documentation_medium.md

### 65. PARTNERSHIP_TYPE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Corporate_partnership_pitches_low.md
- Create_and_Communicate/Content_Generation_from_Scratch/Corporate_partnership_pitches_medium.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_high.md

### 66. AVAILABLE_RESOURCES
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_medium.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_medium.md

### 67. ORGANIZATION_TYPE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Staff_handbooks_and_policy_manuals_high.md
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_medium.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_low.md

### 68. ORGANIZATION_VALUES
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_medium.md

### 69. CORE_MESSAGE
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Cultural_adaptation_of_messaging_low.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_low.md
- Create_and_Communicate/Visual_Content_Creation/Photo_captions_and_alt_text_medium.md

### 70. FOCUS_AREA
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_high.md
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_medium.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Partnership_opportunity_identification_low.md

### 71. SURVEY_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Survey_response_analysis_and_insights_low.md
- Learn_and_Decide/Data_Analysis_&_Insights/Survey_response_analysis_and_insights_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_low.md

### 72. PLANNING_TIMEFRAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_medium.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Strategic_plan_development_and_scenario_planning_medium.md

### 73. CURRENT_CHALLENGES
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_high.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_low.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_medium.md

### 74. PROJECT_NAME
- **Occurrences**: 3
- **Used in**: 3 templates

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Vendor_proposal_evaluation_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_medium.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_low.md

### 75. TIME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 76. STAFF_TITLE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 77. AUDIENCE_DESCRIPTION
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md

### 78. KEY_ACTIVITIES
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md

### 79. START_TIME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 80. COORDINATOR_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md
- Automate_the_Admin/Automated_Communications/Volunteer_shift_reminders_and_updates_high.md

### 81. SIZE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 82. REQUIRED_DOCUMENTS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 83. VALUE_1
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_medium.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 84. VALUE_2
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_medium.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 85. VALUE_3
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_medium.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 86. DONOR_INTERESTS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_low.md

### 87. FUNDING_OPPORTUNITY
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_medium.md
- Automate_the_Admin/Form_&_Document_Review/Grant_application_completeness_checks_low.md

### 88. PROGRAM_DURATION
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 89. MEETING_TIME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 90. MEETING_LOCATION
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 91. SPECIAL_STEPS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_low.md

### 92. JOB_TITLE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md
- Sort_and_Scan/Application_&_Candidate_Screening/Job_applicant_resume_screening_medium.md

### 93. NUMBER_OF_MESSAGES
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_medium.md

### 94. TONE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_low.md

### 95. DEVELOPMENT_DIRECTOR_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 96. FIRST_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_high.md

### 97. EVENT_TIME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 98. RENEWAL_DATE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_medium.md

### 99. EXPIRATION_DATE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_low.md

### 100. MEMBERSHIP_TYPE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_low.md
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_medium.md

### 101. ELIGIBILITY_CRITERIA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Client_intake_form_validation_low.md
- Automate_the_Admin/Form_&_Document_Review/Grant_application_completeness_checks_medium.md

### 102. ANNUAL_BUDGET
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 103. CONTRACT_TYPE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_low.md
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_medium.md

### 104. COMPLIANCE_CHECKLIST
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_low.md
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_low.md

### 105. NEXT_STEPS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_low.md

### 106. ORGANIZATION_LOCATION
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Staff_handbooks_and_policy_manuals_high.md

### 107. RELEVANT_REGULATIONS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 108. TARGET_SYSTEM
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_high.md
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_medium.md

### 109. GRANT_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_medium.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 110. REPORT_TITLE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Creating_executive_summaries_from_detailed_reports_high.md
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Creating_executive_summaries_from_detailed_reports_low.md

### 111. COMPANY_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Corporate_partnership_pitches_low.md
- Create_and_Communicate/Content_Generation_from_Scratch/Corporate_partnership_pitches_medium.md

### 112. DONOR_TYPE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_medium.md
- Create_and_Communicate/Data_Storytelling/Financial_report_explanations_for_donors_low.md

### 113. ISSUE_AREA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Data_Storytelling/Fundraising_case_statements_with_evidence_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Media_coverage_tracking_and_analysis_medium.md

### 114. PROGRAM_DATA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Data_Storytelling/Impact_stories_from_program_data_low.md
- Learn_and_Decide/Data_Analysis_&_Insights/Impact_measurement_and_outcome_tracking_medium.md

### 115. GROUP_SIZE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_low.md
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md

### 116. WEBSITE_URL
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_high.md

### 117. PROCESS_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_low.md
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_medium.md

### 118. VOLUNTEER_ROLE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Volunteer_agreements_and_contracts_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_low.md

### 119. PODCAST_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_high.md
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_medium.md

### 120. TIME_LIMIT
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_low.md

### 121. SPEAKER_ROLE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md

### 122. DESIRED_OUTCOME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_low.md

### 123. KEY_MESSAGE_1
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 124. KEY_MESSAGE_2
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 125. KEY_MESSAGE_3
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 126. PROGRAM_DIRECTOR_NAME
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 127. SOURCE_LANGUAGE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_program_materials_low.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_low.md

### 128. ACCESSIBILITY_NEEDS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_program_materials_low.md
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_medium.md

### 129. BRAND_COLORS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Event_flyers_and_promotional_images_low.md
- Create_and_Communicate/Visual_Content_Creation/Social_media_graphics_and_quote_cards_medium.md

### 130. PRESENTATION_PURPOSE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_high.md
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_medium.md

### 131. SPECIFIC_METRICS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Financial_performance_analysis_low.md
- Learn_and_Decide/Data_Analysis_&_Insights/Program_evaluation_data_analysis_low.md

### 132. IMPACT_GOALS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Impact_measurement_and_outcome_tracking_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Event_planning_proposal_scoring_medium.md

### 133. GEOGRAPHIC_AREA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_high.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_medium.md

### 134. MISSION_AREA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Policy_trend_analysis_and_implications_medium.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_low.md

### 135. PROGRAM_AREA
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_high.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md

### 136. REGULATION_TYPE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_low.md
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md

### 137. PLANNING_OBJECTIVES
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_high.md
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Community_needs_assessment_and_priority_setting_low.md

### 138. RESOURCE_CONSTRAINTS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 139. DECISION_MAKERS
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 140. COMMITTEE_COMPOSITION
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 141. MARKETING_MATERIAL_TYPE
- **Occurrences**: 2
- **Used in**: 2 templates

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_low.md
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_medium.md

### 142. PROGRAM_LOCATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 143. PERCENTAGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 144. KEY_OUTCOME_METRIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 145. BENEFICIARY_FIRST_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 146. BRIEF_TESTIMONIAL_QUOTE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 147. STRATEGIC_GOAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 148. SPECIFIC_PROGRAM_EXAMPLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 149. FUNDRAISING_ROI
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 150. CHARITY_RATING
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 151. RATING_ORGANIZATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 152. CFO_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Donor_inquiries_about_programs_and_impact_high.md

### 153. ATTENDEE_COUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 154. MISSION_ALIGNMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 155. DEMOGRAPHIC_INFORMATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 156. CHECK_IN_TIME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 157. VENUE_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 158. VENUE_ADDRESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 159. STREET_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 160. GPS_ADDRESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 161. VEGETARIAN_MENU_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 162. VEGAN_MENU_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 163. GLUTEN_FREE_MENU_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 164. CATERING_CONTACT_EMAIL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 165. DIETARY_REQUEST_DEADLINE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 166. CONFIRMATION_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 167. SENDER_EMAIL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 168. REGISTRATION_SITE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 169. REGISTRATION_EMAIL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 170. REGISTRATION_PHONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 171. SUPPORT_HOURS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 172. ID_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Event_attendee_logistics_and_information_high.md

### 173. PARTNER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 174. PARTNER_TYPE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 175. PARTNER_TYPE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 176. PARTNER_TYPE_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 177. PARTNER_TYPE_4
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 178. LINK_TO_TEMPLATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 179. LINK_TO_FORMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 180. LINK_TO_PORTAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 181. YOUR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 182. LINK_TO_FLOORPLAN
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 183. LINK_TO_STEPS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 184. LINK_TO_GUIDELINES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 185. LINK_TO_MOU
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 186. LINK_TO_CHECKLIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 187. LINK_TO_SAMPLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Partner_organization_coordination_questions_high.md

### 188. BRIEF_MISSION_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md

### 189. PROGRAM_FOCUS_AREAS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md

### 190. EMAIL_ADDRESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Program_participant_service_questions_high.md

### 191. LIST_DEPARTMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 192. ENTRY_LEVEL_ROLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 193. LEADERSHIP_ROLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 194. GOVERNANCE_STRUCTURE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 195. LIST_POLICY_DOCUMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 196. APPROVAL_PROCESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 197. UPDATE_FREQUENCY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 198. EXCEPTION_APPROVERS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 199. HR_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 200. FINANCE_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 201. PROGRAM_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 202. LEGAL_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 203. EXECUTIVE_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 204. LINK_TO_HR_PORTAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 205. LINK_TO_ANNUAL_CALENDAR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 206. LINK_TO_COVERAGE_PLANNING_TEMPLATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 207. LINK_TO_FORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 208. LINK_TO_PURCHASE_REQUEST_FORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 209. LINK_TO_EXPENSE_REPORT_TEMPLATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 210. LINK_TO_APPROVED_VENDOR_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 211. LINK_TO_PERFORMANCE_DOCUMENTATION_FORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 212. LINK_TO_COACHING_CONVERSATION_GUIDE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_high.md

### 213. POLICY_DOCUMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Staff_questions_about_policies_and_procedures_low.md

### 214. PRIMARY_IMPACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 215. NUMBER_OF_VOLUNTEERS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 216. VOLUNTEER_HOURS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 217. VOLUNTEER_ROLE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 218. BRIEF_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 219. VOLUNTEER_ROLE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 220. VOLUNTEER_ROLE_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 221. MINIMUM_AGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 222. TIME_COMMITMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 223. TRAINING_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 224. REQUIRED_SKILLS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 225. ONBOARDING_STEP_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 226. ONBOARDING_STEP_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 227. ONBOARDING_STEP_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 228. BENEFIT_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 229. BENEFIT_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 230. BENEFIT_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 231. POLICY_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 232. BRIEF_EXPLANATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 233. POLICY_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 234. POLICY_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 235. BRIEF_MISSION_STATEMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 236. APPLICATION_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 237. ORIENTATION_SCHEDULE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 238. DATE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 239. TIME_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 240. DATE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 241. TIME_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 242. MINIMUM_COMMITMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 243. CANCELLATION_NOTICE_PERIOD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 244. RESCHEDULING_METHOD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 245. SUBSTITUTE_POLICY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 246. PORTAL_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 247. SCHEDULER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 248. SCHEDULER_EMAIL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 249. SCHEDULER_PHONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 250. REVIEW_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 251. BACKGROUND_CHECK_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 252. NOTIFICATION_METHOD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 253. HANDBOOK_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 254. VIDEO_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 255. FOLLOW_UP_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 256. ONBOARDING_CONTACT_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/FAQ_&_Knowledge_Base_Responses/Volunteer_program_FAQs_and_onboarding_questions_high.md

### 257. LIST_KEY_PROGRAMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 258. ELIGIBILITY_OVERVIEW
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 259. SERVICE_DELIVERY_MODEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 260. KEY_PARTNERS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 261. FUNDING_LIMITATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 262. CAPACITY_CHALLENGES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 263. CULTURAL_FRAMEWORK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 264. TRAUMA_APPROACH
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 265. COMMON_BARRIERS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_high.md

### 266. CLIENT_NEEDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_low.md

### 267. SUPPORT_PROTOCOLS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Client_service_navigation_and_support_low.md

### 268. RESOURCE_DATABASE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Community_member_resource_referrals_medium.md

### 269. DONOR_HISTORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md

### 270. DONOR_PROFESSION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md

### 271. COMMUNICATION_PREFERENCES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md

### 272. ORGANIZATION_CAPACITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md

### 273. DONOR_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Donor_stewardship_and_engagement_planning_high.md

### 274. SIMILAR_ORGANIZATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 275. RESOURCE_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 276. LOCAL_NONPROFIT_CENTER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 277. ENVIRONMENTAL_EDUCATION_NETWORK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 278. LOCAL_COMMUNITY_FOUNDATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 279. CORPORATE_PARTNER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 280. SIMILAR_FUNDED_PROGRAM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 281. FEDERAL_AGENCY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 282. HEALTHCARE_ASSOCIATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 283. HEALTH_FOUNDATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 284. HEALTHCARE_CORPORATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_high.md

### 285. GRANT_PROGRAM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Grant_application_guidance_and_coaching_low.md

### 286. PROGRAM_FOCUS_AREA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 287. PRIMARY_SERVICES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 288. KEY_RESOURCES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 289. CHALLENGE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 290. CHALLENGE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 291. CHALLENGE_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 292. PARTNER_ORGANIZATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 293. INTERNAL_RESOURCE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 294. INTERNAL_RESOURCE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 295. INTERNAL_RESOURCE_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 296. ESCALATION_TRIGGER_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 297. ESCALATION_TRIGGER_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 298. ESCALATION_TRIGGER_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 299. SUPERVISOR_ROLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 300. ESCALATION_PROCEDURE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_high.md

### 301. ISSUE_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Program_participant_troubleshooting_low.md

### 302. PD_BUDGET
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_high.md

### 303. TALENT_CHALLENGES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_high.md

### 304. SKILL_AREA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_low.md

### 305. STAFF_ROLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_low.md

### 306. CURRENT_SKILL_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_low.md

### 307. CAREER_GOALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Staff_professional_development_guidance_low.md

### 308. TITLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_high.md

### 309. MONTH
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_high.md

### 310. VOLUNTEER_PROFILE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_low.md

### 311. AVAILABLE_POSITIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Answer_and_Assist/Personalized_Support_&_Guidance/Volunteer_placement_and_matching_assistance_low.md

### 312. APPOINTMENT_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 313. MEETING_PURPOSE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 314. ORGANIZATION_ROLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 315. CALENDLY_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 316. RELEVANT_DOCUMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 317. DURATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 318. DISCUSSION_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 319. PROMISED_RESOURCES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 320. ACTION_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 321. DEADLINE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_high.md

### 322. LOCATION_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_low.md

### 323. PREPARATION_STEPS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Appointment_scheduling_and_calendar_management_low.md

### 324. BOARD_CHAIR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 325. VIRTUAL_MEETING_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 326. COMMITTEE_NAMES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 327. BOARD_MEMBER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 328. ORGANIZATION_CULTURE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 329. BOARD_PORTAL_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 330. ZOOM_MEETING_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_high.md

### 331. DOCUMENT_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_low.md

### 332. PRIORITY_AGENDA_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_low.md

### 333. RESPONSE_DEADLINE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_low.md

### 334. DAYS_IN_ADVANCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 335. MATERIALS_DAYS_ADVANCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 336. REMINDER_DAYS_ADVANCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 337. FOLLOW_UP_DAYS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 338. KEY_AGENDA_ITEMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 339. MEETING_MATERIALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Board_meeting_reminders_and_material_distribution_medium.md

### 340. CLIENT_FIRST_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 341. DAY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 342. PROGRAM_PHONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 343. CRISIS_LINE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 344. FACILITATOR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 345. NEXT_SESSION_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 346. EMPLOYER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 347. STARTING_SKILL_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 348. START_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 349. COACH_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 350. LINKEDIN_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 351. DIRECTOR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_high.md

### 352. CLIENT_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md

### 353. PROGRAM_PARTICIPATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md

### 354. LAST_INTERACTION_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md

### 355. COMMUNICATION_CHANNEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_low.md

### 356. MESSAGE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md

### 357. SERVICE_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md

### 358. KEY_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md

### 359. RELATIONSHIP_CONTEXT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md

### 360. ACTION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Client_check-in_and_follow-up_messages_medium.md

### 361. THRESHOLD_AMOUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 362. DONOR_FIRST_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 363. LOCAL_RIVER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 364. NEXT_EVENT_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 365. NEXT_EVENT_LOCATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 366. ED_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 367. SOCIAL_MEDIA_LINKS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 368. HONOREE_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 369. FAMILY_CONTACT_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_high.md

### 370. PROGRAM_SUPPORTED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_low.md

### 371. SMALL_GIFT_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_medium.md

### 372. LARGE_GIFT_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Donor_acknowledgment_and_receipt_automation_medium.md

### 373. LOCAL_FOOD_BANK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 374. DIETARY_NOTES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 375. ACCESSIBILITY_NOTES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 376. EVENT_COORDINATOR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 377. EDUCATION_NONPROFIT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 378. BOOTH_NUMBER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 379. MISSION_STATEMENT_SHORT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 380. CONFERENCE_DIRECTOR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_high.md

### 381. REMINDER_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 382. FINAL_DETAILS_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 383. FOLLOW_UP_ACTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 384. ATTENDEE_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 385. ORGANIZATION_BRANDING
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 386. PARKING_INFO
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 387. WHAT_TO_BRING
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 388. DRESS_CODE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 389. CONTACT_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Event_registration_follow-ups_and_confirmations_medium.md

### 390. MEMBER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md

### 391. MEMBERSHIP_DURATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md

### 392. MEMBERSHIP_TIER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md

### 393. X
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md

### 394. MEMBERSHIP_DIRECTOR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_high.md

### 395. RECENT_ACCOMPLISHMENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_low.md

### 396. PAYMENT_OPTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Membership_renewals_and_payment_reminders_low.md

### 397. PARKING_MAP_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Automated_Communications/Volunteer_shift_reminders_and_updates_high.md

### 398. REGULATORY_BODY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Compliance_reporting_and_form_submissions_high.md

### 399. COMPLIANCE_STANDARDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Compliance_reporting_and_form_submissions_low.md

### 400. COMPLIANCE_STANDARD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Compliance_reporting_and_form_submissions_medium.md

### 401. STAKEHOLDER_ROLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 402. RISK_TOLERANCE_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 403. LEGAL_RESOURCES_AVAILABLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 404. INSURANCE_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 405. INDEMNIFICATION_LANGUAGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 406. PAYMENT_TERMS_GUIDELINES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 407. CONFIDENTIALITY_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 408. IP_STANDARDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 409. TERMINATION_PREFERENCES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 410. APPROVAL_AUTHORITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 411. SIGNATURE_AUTHORITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_high.md

### 412. REVIEW_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_medium.md

### 413. SPECIFIC_POLICIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Contract_review_and_approval_workflows_medium.md

### 414. SPECIFIC_REGULATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Financial_document_verification_low.md

### 415. VERIFICATION_CHECKLIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Financial_document_verification_medium.md

### 416. BUDGET_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Financial_document_verification_medium.md

### 417. FUNDER_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Financial_document_verification_medium.md

### 418. CHECKLIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Grant_application_completeness_checks_low.md

### 419. CHECKLIST_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Grant_application_completeness_checks_medium.md

### 420. FUNDING_PRIORITIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Grant_application_completeness_checks_medium.md

### 421. DOCUMENT_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_low.md

### 422. POLICY_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_low.md

### 423. CRITERION_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 424. STATUS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 425. ISSUES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 426. RECOMMENDATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 427. CRITERION_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 428. DECISION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 429. SUMMARY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 430. ACTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 431. POLICY_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Policy_compliance_auditing_medium.md

### 432. ADDITIONAL_DOCUMENT_TYPES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md

### 433. ADDITIONAL_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md

### 434. PROGRAM_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md

### 435. LEVEL_OF_SUPERVISION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md

### 436. LEVEL_OF_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_high.md

### 437. BACKGROUND_CHECK_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Form_&_Document_Review/Volunteer_background_check_processing_medium.md

### 438. UNIQUE_IDENTIFIER_FIELDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_high.md

### 439. DESTINATION_SYSTEM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_low.md

### 440. DATA_FIELDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Data_entry_between_different_systems_(CRM_to_accounting)_low.md

### 441. REGISTRATION_PLATFORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_high.md

### 442. REGISTRATION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_low.md

### 443. REGISTRATION_CATEGORIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Event_registration_data_processing_medium.md

### 444. BANK_STATEMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 445. ACCOUNTING_SOFTWARE_EXPORTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 446. DONATION_PLATFORM_REPORTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 447. GRANT_EXPENDITURE_LOGS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 448. VARIANCE_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 449. RESTRICTED_FUND_CODES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 450. DONOR_RESTRICTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 451. GRANT_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 452. CHART_OF_ACCOUNTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 453. BOARD_REQUESTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 454. PRIOR_YEAR_ACTUALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_high.md

### 455. FINANCIAL_DOCUMENT_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_low.md

### 456. SOURCE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_low.md

### 457. SOURCE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_low.md

### 458. DATA_SOURCE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md

### 459. DATA_SOURCE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md

### 460. ORGANIZATION_CHART_OF_ACCOUNTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md

### 461. SPENDING_THRESHOLDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Financial_reconciliation_and_reporting_medium.md

### 462. SMALL_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 463. APPROVAL_LEVEL_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 464. LARGE_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 465. APPROVAL_LEVEL_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 466. APPROVAL_LEVEL_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 467. PROGRAM_CODES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 468. ACCOUNTING_SYSTEM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_high.md

### 469. PAYMENT_TERMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_low.md

### 470. DEPARTMENT_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Invoice_processing_and_vendor_management_medium.md

### 471. APPROXIMATE_MEMBER_COUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_high.md

### 472. MEMBERSHIP_LEVELS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_high.md

### 473. SPECIFIC_CHALLENGES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_high.md

### 474. MEMBERSHIP_RULES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_low.md

### 475. DUPLICATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_medium.md

### 476. MISSING
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_medium.md

### 477. EXPIRATION_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_medium.md

### 478. SEGMENTATION_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Membership_data_updates_and_maintenance_medium.md

### 479. ORGANIZATION_RULES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 480. PROGRAM_CATEGORIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 481. VOLUNTEER_ROLES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 482. EVENT_NAMES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 483. VALUE_OF_VOLUNTEER_HOUR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 484. HOURLY_RATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_high.md

### 485. MILESTONE_HOURS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_medium.md

### 486. TRACKING_SYSTEM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Processing_volunteer_timesheets_and_hour_tracking_medium.md

### 487. EXPENSE_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_high.md

### 488. ORGANIZATION_POLICY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_high.md

### 489. EXPENSE_POLICY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_low.md

### 490. APPROVAL_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Automate_the_Admin/Repetitive_Data_Processing/Reviewing_expense_reports_and_receipts_medium.md

### 491. PLATFORM_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_high.md

### 492. PLATFORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Adapting_newsletters_into_social_media_posts_low.md

### 493. TARGET_KEYWORDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_brochures_to_website_copy_high.md

### 494. MAILING_ADDRESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_brochures_to_website_copy_high.md

### 495. WEBSITE_SECTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_brochures_to_website_copy_low.md

### 496. VIDEO_LENGTH
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_written_content_to_video_scripts_high.md

### 497. DISTRIBUTION_PLATFORM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Converting_written_content_to_video_scripts_high.md

### 498. AUDIENCE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Adaptation_&_Reformatting/Creating_executive_summaries_from_detailed_reports_high.md

### 499. SECTOR_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_high.md

### 500. SECTOR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_low.md

### 501. VALUES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_low.md

### 502. ARTICLE_TITLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_medium.md

### 503. MAIN_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Blog_posts_and_thought_leadership_articles_medium.md

### 504. TARGET_COMPANY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Corporate_partnership_pitches_high.md

### 505. EVENT_DATE_TIME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md

### 506. REGISTRATION_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Event_promotional_materials_medium.md

### 507. SPECIFIC_ASK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Newsletter_content_and_email_campaigns_low.md

### 508. PROGRAM_OR_INITIATIVE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Newsletter_content_and_email_campaigns_medium.md

### 509. CITY_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Press_releases_and_media_announcements_high.md

### 510. ANNOUNCEMENT_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Press_releases_and_media_announcements_low.md

### 511. ANNOUNCEMENT_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Press_releases_and_media_announcements_medium.md

### 512. SPOKESPERSON_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Press_releases_and_media_announcements_medium.md

### 513. TARGET_AUDIENCE_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Social_media_posts_and_content_calendars_high.md

### 514. PLATFORM_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Social_media_posts_and_content_calendars_high.md

### 515. PRIMARY_GOAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Social_media_posts_and_content_calendars_medium.md

### 516. SIGNATURE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 517. BRIEF_IMPACT_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 518. SPECIFIC_PROGRAM_OUTCOME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 519. BRIEF_SUCCESS_STORY_OR_STATISTIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 520. SIMPLE_ENGAGEMENT_OPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 521. SENDER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 522. SENDER_TITLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 523. TIMELY_INVITATION_OR_UPDATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 524. DONOR_HISTORY_DESCRIPTOR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 525. SPECIFIC_OUTCOME_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 526. IMPACT_POINT_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 527. IMPACT_POINT_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 528. IMPACT_POINT_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 529. RELEVANT_TESTIMONIAL_OR_STORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 530. PERSONAL_CONNECTION_OR_HISTORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 531. PERSONALIZED_ENGAGEMENT_OFFER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 532. SPECIFIC_FOLLOW_UP_PLAN
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 533. LEADERSHIP_OR_EXCLUSIVE_OPPORTUNITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_high.md

### 534. DONATION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Thank_you_letters_and_donor_communications_medium.md

### 535. SECTION_OR_PROGRAM_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Content_Generation_from_Scratch/Website_copy_and_program_descriptions_medium.md

### 536. FINANCIAL_REPORT_DATA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Data_Storytelling/Financial_report_explanations_for_donors_low.md

### 537. KEY_STATISTICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Data_Storytelling/Fundraising_case_statements_with_evidence_low.md

### 538. SURVEY_DATA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Data_Storytelling/Survey_result_summaries_and_insights_low.md

### 539. SURVEY_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Data_Storytelling/Survey_result_summaries_and_insights_medium.md

### 540. PURPOSE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Survey_design_and_evaluation_forms_low.md

### 541. SPECIFIC_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Survey_design_and_evaluation_forms_low.md

### 542. SURVEY_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Survey_design_and_evaluation_forms_medium.md

### 543. LEARNING_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md

### 544. SKILL_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md

### 545. TIME_CONSTRAINT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md

### 546. ENGAGEMENT_STYLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Interactive_Content_Creation/Training_curricula_and_workshop_activities_medium.md

### 547. EMERGENCY_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Safety_protocols_and_emergency_procedures_low.md

### 548. LOCATION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Safety_protocols_and_emergency_procedures_low.md

### 549. VULNERABLE_POPULATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Safety_protocols_and_emergency_procedures_low.md

### 550. SAFETY_CONCERN_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Safety_protocols_and_emergency_procedures_medium.md

### 551. FACILITY_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Safety_protocols_and_emergency_procedures_medium.md

### 552. PHONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 553. TECH_SUPPORT_EMAIL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 554. FILE_LOCATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 555. FUNDER_CONTACT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 556. DATABASE_ADMIN
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 557. CONTACT_INFO
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_high.md

### 558. PROCEDURE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Training_materials_and_onboarding_guides_low.md

### 559. STATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Volunteer_agreements_and_contracts_high.md

### 560. VOLUNTEER_ROLE_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Policy_&_Procedure_Documentation/Volunteer_agreements_and_contracts_medium.md

### 561. EPISODE_LENGTH
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_medium.md

### 562. EPISODE_TOPIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_medium.md

### 563. GUEST_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_medium.md

### 564. GUEST_ORGANIZATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Podcast_episode_outlines_and_show_notes_medium.md

### 565. KEY_PROGRAMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 566. IMPACT_METRICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 567. EVENT_THEME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 568. TECHNICAL_SETUP
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 569. AUDIENCE_KNOWLEDGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 570. AUDIENCE_SIZE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 571. AUDIENCE_EXPECTATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 572. SPEAKER_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 573. SPEAKER_BACKGROUND
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 574. SPEAKING_STYLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 575. PRIMARY_OBJECTIVE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 576. KEY_MESSAGES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 577. SUCCESS_METRICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_high.md

### 578. AUDIENCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_low.md

### 579. LENGTH_IN_MINUTES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Speech_writing_for_events_and_presentations_medium.md

### 580. VIDEO_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_low.md

### 581. KEY_POINTS_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Script_&_Narrative_Writing/Video_scripts_for_programs_and_testimonials_medium.md

### 582. YEAR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_low.md

### 583. PROGRAM_RESULTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_medium.md

### 584. FINANCIAL_INFORMATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_medium.md

### 585. BENEFICIARY_STORIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_medium.md

### 586. DONOR_RECOGNITION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Annual_reports_and_impact_summaries_medium.md

### 587. SPECIFIC_TOPICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Board_reports_and_meeting_materials_low.md

### 588. SWOT_ANALYSIS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Structured_Report_Creation/Strategic_planning_documents_medium.md

### 589. RECIPIENT_NAMES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_low.md

### 590. ACHIEVEMENT_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_low.md

### 591. SPECIAL_FORMATTING
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_low.md

### 592. DATE_FORMAT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_low.md

### 593. CERTIFICATE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_medium.md

### 594. NUMBER_OF_CERTIFICATES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_medium.md

### 595. ACHIEVEMENT_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Certificate_and_award_generation_medium.md

### 596. CREDENTIAL_EVALUATION_SERVICE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_high.md

### 597. TEMPLATE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_low.md

### 598. CLIENT_DETAILS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_low.md

### 599. SERVICE_PROVIDED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Client_service_communications_and_follow-ups_low.md

### 600. SPECIAL_NOTES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 601. TICKET_PRICE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 602. RSVP_DEADLINE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 603. CONTACT_PERSON
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 604. RSVP_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 605. TICKET_COUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 606. COST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 607. SURVEY_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_high.md

### 608. INVITATION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Event_invitations_and_confirmations_medium.md

### 609. TARGET_NEIGHBORHOOD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 610. BUILDING_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 611. DISASTER_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_high.md

### 612. GIVING_HISTORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_low.md

### 613. PERSONAL_CONNECTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_low.md

### 614. PAST_INTERACTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_low.md

### 615. TONE_AND_VOICE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Personalized_donor_appeal_letters_low.md

### 616. SCHEDULING_LINK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 617. ORIENTATION_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 618. ORIENTATION_TIME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 619. LOCATION_ADDRESS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_high.md

### 620. TARGET_VOLUNTEER_DEMOGRAPHIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_low.md

### 621. KEY_BENEFITS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Template_Filling_&_Personalization/Volunteer_recruitment_and_onboarding_communications_low.md

### 622. SOURCE_CONTENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Cultural_adaptation_of_messaging_medium.md

### 623. ACCESSIBILITY_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Cultural_adaptation_of_messaging_medium.md

### 624. ACCESSIBILITY_NEED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_program_materials_medium.md

### 625. ACCESSIBILITY_FORMAT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_high.md

### 626. SPECIFIC_DATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_high.md

### 627. TIMEZONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_high.md

### 628. ORIGINAL_CONTENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Multi-language_social_media_content_medium.md

### 629. READING_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Translation_&_Accessibility_Conversion/Plain_language_versions_of_complex_documents_medium.md

### 630. LOGO_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Event_flyers_and_promotional_images_low.md

### 631. COLOR1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Infographics_about_programs_and_impact_low.md

### 632. COLOR2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Infographics_about_programs_and_impact_low.md

### 633. STATISTIC1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Infographics_about_programs_and_impact_low.md

### 634. STATISTIC2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Infographics_about_programs_and_impact_low.md

### 635. ORGANIZATION_TONE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Photo_captions_and_alt_text_medium.md

### 636. SLIDE_COUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_medium.md

### 637. ORGANIZATION_COLORS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_medium.md

### 638. VIEWING_CONTEXT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Presentation_slide_designs_medium.md

### 639. FOOD_SECURITY_NONPROFIT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Social_media_graphics_and_quote_cards_high.md

### 640. YOUTH_MENTORSHIP_PROGRAM
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Create_and_Communicate/Visual_Content_Creation/Social_media_graphics_and_quote_cards_high.md

### 641. STRATEGIC_PRIORITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_high.md

### 642. RESOURCE_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_high.md

### 643. DATA_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_medium.md

### 644. COMMUNITY_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Community_needs_assessment_data_interpretation_medium.md

### 645. SPECIFIC_OBJECTIVE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Donor_behavior_and_giving_pattern_analysis_low.md

### 646. BENCHMARK_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Financial_performance_analysis_medium.md

### 647. METRICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Impact_measurement_and_outcome_tracking_low.md

### 648. TARGET_METRICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Impact_measurement_and_outcome_tracking_medium.md

### 649. PROGRAM_GOALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Program_evaluation_data_analysis_medium.md

### 650. VOLUNTEER_DATA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Data_Analysis_&_Insights/Volunteer_engagement_and_retention_analysis_low.md

### 651. BUDGET_RANGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_high.md

### 652. ORGANIZATIONAL_CONSTRAINTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Best_practices_research_for_program_development_medium.md

### 653. PROGRAM_AREAS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Funding_opportunity_research_and_matching_low.md

### 654. GEOGRAPHIC_LEVEL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Policy_trend_analysis_and_implications_medium.md

### 655. YOUTH_LEARNING_ALLIANCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_high.md

### 656. GLOBAL_RELIEF_NETWORK
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_high.md

### 657. REGULATION_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md

### 658. JURISDICTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md

### 659. SPECIFIC_CONCERN
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Regulatory_compliance_research_and_updates_medium.md

### 660. SPECIFIC_FUNCTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_high.md

### 661. APPENDIX
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_high.md

### 662. TECHNOLOGY_SOLUTION_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 663. SPECIFIC_SOLUTION_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 664. SPECIFIC_SOLUTION_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 665. SPECIFIC_SOLUTION_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 666. BUDGET_AMOUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 667. PRIMARY_NEED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_low.md

### 668. NUMBER_OF_SOLUTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_medium.md

### 669. TECHNOLOGY_NEED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Research_&_Intelligence_Gathering/Technology_solution_evaluation_and_comparison_medium.md

### 670. FORECAST_PERIOD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_low.md

### 671. SPECIFIC_FINANCIAL_GOALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_low.md

### 672. FINANCIAL_DATA_TIMEFRAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_medium.md

### 673. NUMBER_OF_SCENARIOS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_medium.md

### 674. KEY_ASSUMPTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Budget_analysis_and_financial_forecasting_medium.md

### 675. MISSION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_high.md

### 676. PROGRAMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_high.md

### 677. KEY_PRIORITIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_low.md

### 678. GROWTH_INDICATORS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_low.md

### 679. CURRENT_PROGRAMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 680. STAFF_CAPACITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 681. FUNDING_SOURCES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 682. KEY_CHALLENGES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 683. EXISTING_STRENGTHS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 684. STRATEGIC_OBJECTIVES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Capacity_building_and_organizational_development_medium.md

### 685. CURRENT_DATA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Community_needs_assessment_and_priority_setting_low.md

### 686. PROGRAM_METRICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Impact_projection_and_theory_of_change_development_low.md

### 687. TARGET_OUTCOMES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Impact_projection_and_theory_of_change_development_low.md

### 688. TARGET_REGION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Program_expansion_feasibility_analysis_low.md

### 689. ORGANIZATIONAL_GOALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_low.md

### 690. STAKEHOLDERS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_medium.md

### 691. REGULATORY_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Risk_assessment_and_mitigation_planning_medium.md

### 692. TARGET_BENEFICIARIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Learn_and_Decide/Strategic_Planning_&_Forecasting/Strategic_plan_development_and_scenario_planning_high.md

### 693. BOARD_NEEDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Board_member_candidate_assessment_low.md

### 694. FOCUS_ISSUE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 695. GRANT_POOL_SIZE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 696. PRIORITY_AREA_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 697. PRIORITY_AREA_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 698. PRIORITY_AREA_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 699. EQUITY_FOCUS_GROUPS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 700. NEXT_REVIEW_STAGE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 701. LEGAL_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 702. REPORTING_EXPECTATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_high.md

### 703. MINIMUM_SCORE_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Grant_application_initial_screening_low.md

### 704. POSITION_TITLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Job_applicant_resume_screening_low.md

### 705. KEY_REQUIREMENT_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Job_applicant_resume_screening_low.md

### 706. KEY_REQUIREMENT_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Job_applicant_resume_screening_low.md

### 707. MINIMUM_SCORE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 708. PROGRAM_CAPACITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 709. PROGRAM_OBJECTIVE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 710. DECISION_MAKER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 711. PROGRAM_BENEFITS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 712. PARTICIPANT_EXPECTATIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 713. ORGANIZATIONAL_VALUES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_high.md

### 714. KEY_QUALITY_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_low.md
### 714. KEY_QUALITY_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_low.md

### 715. KEY_QUALITY_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_low.md

### 716. CORE_VALUE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Program_participant_selection_low.md

### 717. SCHOLARSHIP_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 718. X_NUMBER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 719. SCHOLARSHIP_PURPOSE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 720. QUALITIES_OF_SUCCESSFUL_RECIPIENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 721. COMMITTEE_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 722. OTHER_DECISION_FACTORS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_high.md

### 723. SCHOLARSHIP_MISSION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_low.md

### 724. EVALUATION_RUBRIC
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Scholarship_candidate_evaluation_medium.md

### 725. VENDOR_PROPOSALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Vendor_proposal_evaluation_low.md

### 726. KEY_SKILL_REQUIREMENTS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Volunteer_application_processing_high.md

### 727. RETENTION_RATE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Volunteer_application_processing_high.md

### 728. SPECIFIC_NEEDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Volunteer_application_processing_low.md

### 729. PROGRAM_OR_ROLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Application_&_Candidate_Screening/Volunteer_application_processing_low.md

### 730. ENGAGEMENT_ACTIVITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_high.md

### 731. PRIMARY_CATEGORY_SET
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_high.md

### 732. PRIORITIZATION_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_low.md

### 733. CATEGORY_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_medium.md

### 734. CATEGORY_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_medium.md

### 735. CATEGORY_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Community_input_organization_and_themes_medium.md

### 736. DONOR_LIST
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Donor_prospect_identification_and_ranking_low.md

### 737. PROGRAM_FOCUS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Donor_prospect_identification_and_ranking_medium.md

### 738. PRIORITY_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 739. PRIORITY_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 740. PRIORITY_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 741. CAMPAIGN_PHASE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 742. CURRENT_CAMPAIGN
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 743. STAKEHOLDER_GROUPS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 744. DEPARTMENT_COUNT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 745. RESPONSE_CAPACITY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 746. BUSY_PERIOD_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 747. BUSY_PERIOD_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 748. SPECIAL_CONSIDERATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_high.md

### 749. URGENT_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 750. HIGH_PRIORITY_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 751. MEDIUM_PRIORITY_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 752. LOW_PRIORITY_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 753. AUTOMATED_RESPONSE_CATEGORY
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 754. DEPARTMENT_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 755. DEPARTMENT_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Email_triage_and_response_prioritization_medium.md

### 756. STAKEHOLDER_TYPES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Feedback_and_complaint_categorization_high.md

### 757. FEEDBACK_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Feedback_and_complaint_categorization_low.md

### 758. KEY_VALUES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Feedback_and_complaint_categorization_low.md

### 759. ORGANIZATION_SECTOR
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Media_coverage_tracking_and_analysis_low.md

### 760. PROGRAM_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 761. PROGRAM_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 762. PROGRAM_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 763. CAMPAIGN_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 764. CAMPAIGN_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 765. RECENT_NEWS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 766. VOICE_CHARACTERISTICS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 767. TONE_GUIDELINES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 768. HASHTAG_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 769. HASHTAG_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 770. HASHTAG_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 771. TIMEFRAME_GOALS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 772. ROLE_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 773. TYPE_OF_MENTIONS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 774. ROLE_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 775. ROLE_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 776. BRACKETED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_high.md

### 777. TONE_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Social_media_mention_monitoring_and_response_low.md

### 778. NUMBER_OF_CATEGORIES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_high.md

### 779. CATEGORIZATION_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_medium.md

### 780. THEME_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_medium.md

### 781. THEME_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_medium.md

### 782. THEME_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Content_Categorization_&_Prioritization/Survey_response_categorization_and_analysis_medium.md

### 783. SPECIFIC_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Content_quality_assurance_and_editing_low.md

### 784. BUDGET_CONSTRAINT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Event_planning_proposal_scoring_medium.md

### 785. MATERIAL_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_high.md

### 786. MATERIAL_CONTENT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_high.md

### 787. CAMPAIGN_NAME
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Marketing_material_effectiveness_review_low.md

### 788. PARTNER_ORGANIZATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_low.md

### 789. ASSESSMENT_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_low.md

### 790. SCORE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 791. BRIEF_JUSTIFICATION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 792. ADDITIONAL_CRITERION_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 793. ADDITIONAL_CRITERION_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 794. TOTAL_SCORE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 795. MAXIMUM_POSSIBLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 796. PARTNERSHIP_TIER
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 797. STRENGTH_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 798. STRENGTH_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 799. STRENGTH_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 800. RECOMMENDATION_1
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 801. SPECIFIC_ACTION_STEP
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 802. RECOMMENDATION_2
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 803. RECOMMENDATION_3
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 804. BRIEF_NEXT_STEPS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Partner_organization_capability_assessment_medium.md

### 805. COMMUNITY_NEED
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 806. EXISTING_PROGRAMS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 807. PREVIOUS_OUTCOMES
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 808. RUBRIC_SOURCE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 809. MIN_SCORE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 810. MAX_SCORE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 811. PASSING_THRESHOLD
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 812. PRIORITY_CRITERIA
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_high.md

### 813. PROGRAM_PROPOSAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_low.md

### 814. PROPOSAL_TEXT
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Program_proposal_evaluation_and_feedback_medium.md

### 815. SERVICE_DESCRIPTION
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Service_delivery_quality_evaluation_low.md

### 816. QUALITY_STANDARDS
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Service_delivery_quality_evaluation_low.md

### 817. MATERIAL_TITLE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Training_material_quality_assessment_high.md

### 818. TRAINING_MATERIAL
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Training_material_quality_assessment_low.md

### 819. TRAINING_MATERIAL_TYPE
- **Occurrences**: 1
- **Used in**: 1 template

**Files:**
- Sort_and_Scan/Quality_Assessment_&_Scoring/Training_material_quality_assessment_medium.md

## Most Common Variables (Top 20)

| Rank | Variable | Count | Usage Description |
|------|----------|-------|------------------|
| 1 | `ORGANIZATION_NAME` | 353 | Organization identification |
| 2 | `TARGET_AUDIENCE` | 89 | Unknown |
| 3 | `PROGRAM_NAME` | 83 | Program/service reference |
| 4 | `MISSION_STATEMENT` | 65 | Unknown |
| 5 | `EVENT_NAME` | 20 | Name field |
| 6 | `TARGET_POPULATION` | 16 | Unknown |
| 7 | `DOCUMENT_TYPE` | 12 | Unknown |
| 8 | `TIME_PERIOD` | 11 | Unknown |
| 9 | `DONOR_NAME` | 10 | Name field |
| 10 | `EVENT_DATE` | 9 | Date/time reference |
| 11 | `TOPIC` | 9 | Unknown |
| 12 | `NUMBER` | 8 | Unknown |
| 13 | `EVENT_TYPE` | 8 | Event reference |
| 14 | `CONTENT_TYPE` | 8 | Unknown |
| 15 | `DATE` | 7 | Date/time reference |
| 16 | `LOCATION` | 7 | Unknown |
| 17 | `DONATION_AMOUNT` | 7 | Financial amount |
| 18 | `MISSION_FOCUS` | 7 | Unknown |
| 19 | `TIMEFRAME` | 6 | Unknown |
| 20 | `PHONE_NUMBER` | 6 | Phone number |

## Variable Categories

### Organization Identity
- `ORGANIZATION_NAME` (353)
- `ORGANIZATION_MISSION` (4)
- `ORGANIZATION_TYPE` (3)
- `ORGANIZATION_VALUES` (3)
- `ORGANIZATION_LOCATION` (2)
- `RATING_ORGANIZATION` (1)
- `ORGANIZATION_CAPACITY` (1)
- `SIMILAR_ORGANIZATION` (1)
- `PARTNER_ORGANIZATIONS` (1)
- `ORGANIZATION_ROLE` (1)
- `ORGANIZATION_CULTURE` (1)
- `ORGANIZATION_BRANDING` (1)
- `ORGANIZATION_CHART_OF_ACCOUNTS` (1)
- `ORGANIZATION_RULES` (1)
- `ORGANIZATION_POLICY` (1)
- `GUEST_ORGANIZATION` (1)
- `ORGANIZATION_TONE` (1)
- `ORGANIZATION_COLORS` (1)
- `ORGANIZATIONAL_CONSTRAINTS` (1)
- `ORGANIZATIONAL_GOALS` (1)
- `ORGANIZATIONAL_VALUES` (1)
- `ORGANIZATION_SECTOR` (1)
- `PARTNER_ORGANIZATION` (1)

### Programs & Services
- `PROGRAM_NAME` (83)
- `PROGRAM_TYPE` (4)
- `SERVICE_TYPE` (3)
- `SERVICE_AREA` (3)
- `PROGRAM_DURATION` (2)
- `PROGRAM_DATA` (2)
- `PROGRAM_DIRECTOR_NAME` (2)
- `PROGRAM_AREA` (2)
- `PROGRAM_LOCATION` (1)
- `SPECIFIC_PROGRAM_EXAMPLES` (1)
- `PROGRAM_FOCUS_AREAS` (1)
- `PROGRAM_CONTACT` (1)
- `LIST_KEY_PROGRAMS` (1)
- `SERVICE_DELIVERY_MODEL` (1)
- `SIMILAR_FUNDED_PROGRAM` (1)
- `GRANT_PROGRAM` (1)
- `PROGRAM_FOCUS_AREA` (1)
- `PRIMARY_SERVICES` (1)
- `PROGRAM_PHONE` (1)
- `PROGRAM_PARTICIPATION` (1)
- `SERVICE_NAME` (1)
- `PROGRAM_SUPPORTED` (1)
- `PROGRAM_DESCRIPTION` (1)
- `PROGRAM_CODES` (1)
- `PROGRAM_CATEGORIES` (1)
- `PROGRAM_OR_INITIATIVE` (1)
- `SPECIFIC_PROGRAM_OUTCOME` (1)
- `SECTION_OR_PROGRAM_NAME` (1)
- `KEY_PROGRAMS` (1)
- `PROGRAM_RESULTS` (1)
- `CREDENTIAL_EVALUATION_SERVICE` (1)
- `SERVICE_PROVIDED` (1)
- `YOUTH_MENTORSHIP_PROGRAM` (1)
- `PROGRAM_GOALS` (1)
- `PROGRAM_AREAS` (1)
- `PROGRAMS` (1)
- `CURRENT_PROGRAMS` (1)
- `PROGRAM_METRICS` (1)
- `PROGRAM_CAPACITY` (1)
- `PROGRAM_OBJECTIVE` (1)
- `PROGRAM_BENEFITS` (1)
- `PROGRAM_OR_ROLE` (1)
- `PROGRAM_FOCUS` (1)
- `PROGRAM_1` (1)
- `PROGRAM_2` (1)
- `PROGRAM_3` (1)
- `EXISTING_PROGRAMS` (1)
- `PROGRAM_PROPOSAL` (1)
- `SERVICE_DESCRIPTION` (1)

### People & Contacts
- `EVENT_NAME` (20)
- `DONOR_NAME` (10)
- `PHONE_NUMBER` (6)
- `FUNDER_NAME` (5)
- `CONTACT_INFORMATION` (4)
- `PLATFORM_NAME` (4)
- `STAFF_NAME` (3)
- `EMAIL` (3)
- `VOLUNTEER_COORDINATOR_NAME` (3)
- `COORDINATOR_EMAIL` (3)
- `COORDINATOR_PHONE` (3)
- `CONTACT_EMAIL` (3)
- `CONTACT_PHONE` (3)
- `STAFF_SIZE` (3)
- `CLIENT_NAME` (3)
- `RECIPIENT_NAME` (3)
- `EXECUTIVE_DIRECTOR_NAME` (3)
- `SURVEY_NAME` (3)
- `PROJECT_NAME` (3)
- `STAFF_TITLE` (2)
- `COORDINATOR_NAME` (2)
- `DONOR_INTERESTS` (2)
- `DEVELOPMENT_DIRECTOR_NAME` (2)
- `FIRST_NAME` (2)
- `GRANT_NAME` (2)
- `COMPANY_NAME` (2)
- `DONOR_TYPE` (2)
- `PROCESS_NAME` (2)
- `VOLUNTEER_ROLE` (2)
- `PODCAST_NAME` (2)
- `BENEFICIARY_FIRST_NAME` (1)
- `CFO_NAME` (1)
- `VENUE_NAME` (1)
- `VENUE_ADDRESS` (1)
- `STREET_NAME` (1)
- `GPS_ADDRESS` (1)
- `CATERING_CONTACT_EMAIL` (1)
- `SENDER_EMAIL` (1)
- `REGISTRATION_EMAIL` (1)
- `REGISTRATION_PHONE` (1)
- `PARTNER_NAME` (1)
- `YOUR_NAME` (1)
- `EMAIL_ADDRESS` (1)
- `HR_CONTACT` (1)
- `FINANCE_CONTACT` (1)
- `LEGAL_CONTACT` (1)
- `EXECUTIVE_CONTACT` (1)
- `NUMBER_OF_VOLUNTEERS` (1)
- `VOLUNTEER_HOURS` (1)
- `VOLUNTEER_ROLE_1` (1)
- `VOLUNTEER_ROLE_2` (1)
- `VOLUNTEER_ROLE_3` (1)
- `ONBOARDING_STEP_1` (1)
- `ONBOARDING_STEP_2` (1)
- `ONBOARDING_STEP_3` (1)
- `SCHEDULER_NAME` (1)
- `SCHEDULER_EMAIL` (1)
- `SCHEDULER_PHONE` (1)
- `ONBOARDING_CONTACT_NAME` (1)
- `CLIENT_NEEDS` (1)
- `DONOR_HISTORY` (1)
- `DONOR_PROFESSION` (1)
- `DONOR_CONTACT` (1)
- `STAFF_ROLE` (1)
- `VOLUNTEER_PROFILE` (1)
- `BOARD_CHAIR_NAME` (1)
- `COMMITTEE_NAMES` (1)
- `BOARD_MEMBER_NAME` (1)
- `BOARD_PORTAL_LINK` (1)
- `CLIENT_FIRST_NAME` (1)
- `FACILITATOR_NAME` (1)
- `EMPLOYER_NAME` (1)
- `COACH_NAME` (1)
- `DIRECTOR_NAME` (1)
- `CLIENT_TYPE` (1)
- `DONOR_FIRST_NAME` (1)
- `LOCAL_RIVER_NAME` (1)
- `ED_NAME` (1)
- `HONOREE_NAME` (1)
- `FAMILY_CONTACT_NAME` (1)
- `EVENT_COORDINATOR_NAME` (1)
- `ATTENDEE_NAME` (1)
- `CONTACT_NAME` (1)
- `MEMBER_NAME` (1)
- `MEMBERSHIP_DIRECTOR_NAME` (1)
- `CHECKLIST_NAME` (1)
- `DOCUMENT_NAME` (1)
- `POLICY_NAME` (1)
- `LEVEL_OF_CONTACT` (1)
- `DONOR_RESTRICTIONS` (1)
- `BOARD_REQUESTS` (1)
- `DEPARTMENT_NAME` (1)
- `VOLUNTEER_ROLES` (1)
- `EVENT_NAMES` (1)
- `VALUE_OF_VOLUNTEER_HOUR` (1)
- `MAILING_ADDRESS` (1)
- `SECTOR_NAME` (1)
- `CITY_NAME` (1)
- `SPOKESPERSON_NAME` (1)
- `SENDER_NAME` (1)
- `DONOR_HISTORY_DESCRIPTOR` (1)
- `PERSONAL_CONNECTION_OR_HISTORY` (1)
- `PERSONALIZED_ENGAGEMENT_OFFER` (1)
- `PHONE` (1)
- `TECH_SUPPORT_EMAIL` (1)
- `FUNDER_CONTACT` (1)
- `CONTACT_INFO` (1)
- `VOLUNTEER_ROLE_DESCRIPTION` (1)
- `GUEST_NAME` (1)
- `SPEAKER_NAME` (1)
- `DONOR_RECOGNITION` (1)
- `RECIPIENT_NAMES` (1)
- `CLIENT_DETAILS` (1)
- `CONTACT_PERSON` (1)
- `BUILDING_NAME` (1)
- `PERSONAL_CONNECTION` (1)
- `LOCATION_ADDRESS` (1)
- `TARGET_VOLUNTEER_DEMOGRAPHIC` (1)
- `COMMUNITY_NAME` (1)
- `VOLUNTEER_DATA` (1)
- `REGULATION_NAME` (1)
- `STAFF_CAPACITY` (1)
- `BOARD_NEEDS` (1)
- `PARTICIPANT_EXPECTATIONS` (1)
- `SCHOLARSHIP_NAME` (1)
- `COMMITTEE_NAME` (1)
- `DONOR_LIST` (1)
- `MATERIAL_NAME` (1)
- `CAMPAIGN_NAME` (1)

### Events & Activities
- `EVENT_DATE` (9)
- `EVENT_TYPE` (8)
- `EVENT_LOCATION` (5)
- `EVENT_PURPOSE` (4)
- `MEETING_DATE` (3)
- `MEETING_TIME` (2)
- `MEETING_LOCATION` (2)
- `EVENT_TIME` (2)
- `MEETING_PURPOSE` (1)
- `VIRTUAL_MEETING_LINK` (1)
- `ZOOM_MEETING_LINK` (1)
- `MEETING_MATERIALS` (1)
- `NEXT_EVENT_DATE` (1)
- `NEXT_EVENT_LOCATION` (1)
- `EVENT_DATE_TIME` (1)
- `EVENT_THEME` (1)
- `ENGAGEMENT_ACTIVITY` (1)

### Financial
- `DONATION_AMOUNT` (7)
- `AMOUNT` (3)
- `FUNDING_OPPORTUNITY` (2)
- `ANNUAL_BUDGET` (2)
- `FUNDRAISING_ROI` (1)
- `FUNDING_LIMITATIONS` (1)
- `PD_BUDGET` (1)
- `THRESHOLD_AMOUNT` (1)
- `BUDGET_CATEGORY` (1)
- `FUNDER_REQUIREMENTS` (1)
- `FUNDING_PRIORITIES` (1)
- `RESTRICTED_FUND_CODES` (1)
- `FINANCIAL_DOCUMENT_TYPE` (1)
- `FINANCIAL_REPORT_DATA` (1)
- `FINANCIAL_INFORMATION` (1)
- `TICKET_PRICE` (1)
- `COST` (1)
- `BUDGET_RANGE` (1)
- `BUDGET_AMOUNT` (1)
- `SPECIFIC_FINANCIAL_GOALS` (1)
- `FINANCIAL_DATA_TIMEFRAME` (1)
- `FUNDING_SOURCES` (1)
- `BUDGET_CONSTRAINT` (1)

### Dates & Times
- `TIME_PERIOD` (11)
- `DATE` (7)
- `TIMEFRAME` (6)
- `PLANNING_TIMEFRAME` (3)
- `TIME` (2)
- `START_TIME` (2)
- `RENEWAL_DATE` (2)
- `EXPIRATION_DATE` (2)
- `TIME_LIMIT` (2)
- `CHECK_IN_TIME` (1)
- `DIETARY_REQUEST_DEADLINE` (1)
- `CONFIRMATION_TIMEFRAME` (1)
- `UPDATE_FREQUENCY` (1)
- `TIME_COMMITMENT` (1)
- `ORIENTATION_SCHEDULE` (1)
- `DATE_1` (1)
- `TIME_1` (1)
- `DATE_2` (1)
- `TIME_2` (1)
- `REVIEW_TIMEFRAME` (1)
- `BACKGROUND_CHECK_TIMEFRAME` (1)
- `FOLLOW_UP_TIMEFRAME` (1)
- `DEADLINE` (1)
- `RESPONSE_DEADLINE` (1)
- `NEXT_SESSION_DATE` (1)
- `START_DATE` (1)
- `LAST_INTERACTION_DATE` (1)
- `REMINDER_TIMEFRAME` (1)
- `FINAL_DETAILS_TIMEFRAME` (1)
- `TIMELY_INVITATION_OR_UPDATE` (1)
- `TIME_CONSTRAINT` (1)
- `DATE_FORMAT` (1)
- `RSVP_DEADLINE` (1)
- `ORIENTATION_DATE` (1)
- `ORIENTATION_TIME` (1)
- `SPECIFIC_DATE` (1)
- `TIMEZONE` (1)
- `TIMEFRAME_GOALS` (1)

### Communications
- `KEY_MESSAGE` (5)
- `CORE_MESSAGE` (3)
- `NUMBER_OF_MESSAGES` (2)
- `KEY_MESSAGE_1` (2)
- `KEY_MESSAGE_2` (2)
- `KEY_MESSAGE_3` (2)
- `COMMUNICATION_PREFERENCES` (1)
- `COMMUNICATION_CHANNEL` (1)
- `MESSAGE_TYPE` (1)
- `SOCIAL_MEDIA_LINKS` (1)
- `KEY_MESSAGES` (1)

### Other
- `TARGET_AUDIENCE` (89)
- `MISSION_STATEMENT` (65)
- `TARGET_POPULATION` (16)
- `DOCUMENT_TYPE` (12)
- `TOPIC` (9)
- `NUMBER` (8)
- `CONTENT_TYPE` (8)
- `LOCATION` (7)
- `MISSION_FOCUS` (7)
- `TARGET_LANGUAGE` (6)
- `BRACKETS` (5)
- `WORD_COUNT` (5)
- `POLICY_AREA` (5)
- `CRITERIA_LIST` (5)
- `STRATEGIC_PRIORITIES` (4)
- `BRACKETED_VARIABLES` (4)
- `LINK` (4)
- `DATA_SOURCE` (4)
- `CAUSE_AREA` (4)
- `SECTOR_TYPE` (4)
- `DESIRED_ACTION` (4)
- `LEARNING_OBJECTIVES` (4)
- `EVALUATION_CRITERIA` (4)
- `CORE_VALUES` (3)
- `SOURCE_SYSTEM` (3)
- `CRM_SYSTEM` (3)
- `REPORTING_PERIOD` (3)
- `STAKEHOLDER_TYPE` (3)
- `LENGTH` (3)
- `REPORT_TYPE` (3)
- `PARTNERSHIP_TYPE` (3)
- `AVAILABLE_RESOURCES` (3)
- `FOCUS_AREA` (3)
- `CURRENT_CHALLENGES` (3)
- `AUDIENCE_DESCRIPTION` (2)
- `KEY_ACTIVITIES` (2)
- `SIZE` (2)
- `REQUIRED_DOCUMENTS` (2)
- `VALUE_1` (2)
- `VALUE_2` (2)
- `VALUE_3` (2)
- `SPECIAL_STEPS` (2)
- `JOB_TITLE` (2)
- `TONE` (2)
- `MEMBERSHIP_TYPE` (2)
- `ELIGIBILITY_CRITERIA` (2)
- `CONTRACT_TYPE` (2)
- `COMPLIANCE_CHECKLIST` (2)
- `NEXT_STEPS` (2)
- `RELEVANT_REGULATIONS` (2)
- `TARGET_SYSTEM` (2)
- `REPORT_TITLE` (2)
- `ISSUE_AREA` (2)
- `GROUP_SIZE` (2)
- `WEBSITE_URL` (2)
- `SPEAKER_ROLE` (2)
- `DESIRED_OUTCOME` (2)
- `SOURCE_LANGUAGE` (2)
- `ACCESSIBILITY_NEEDS` (2)
- `BRAND_COLORS` (2)
- `PRESENTATION_PURPOSE` (2)
- `SPECIFIC_METRICS` (2)
- `IMPACT_GOALS` (2)
- `GEOGRAPHIC_AREA` (2)
- `MISSION_AREA` (2)
- `REGULATION_TYPE` (2)
- `PLANNING_OBJECTIVES` (2)
- `RESOURCE_CONSTRAINTS` (2)
- `DECISION_MAKERS` (2)
- `COMMITTEE_COMPOSITION` (2)
- `MARKETING_MATERIAL_TYPE` (2)
- `PERCENTAGE` (1)
- `KEY_OUTCOME_METRIC` (1)
- `BRIEF_TESTIMONIAL_QUOTE` (1)
- `STRATEGIC_GOAL` (1)
- `CHARITY_RATING` (1)
- `ATTENDEE_COUNT` (1)
- `MISSION_ALIGNMENT` (1)
- `DEMOGRAPHIC_INFORMATION` (1)
- `VEGETARIAN_MENU_ITEMS` (1)
- `VEGAN_MENU_ITEMS` (1)
- `GLUTEN_FREE_MENU_ITEMS` (1)
- `REGISTRATION_SITE` (1)
- `SUPPORT_HOURS` (1)
- `ID_REQUIREMENTS` (1)
- `PARTNER_TYPE_1` (1)
- `PARTNER_TYPE_2` (1)
- `PARTNER_TYPE_3` (1)
- `PARTNER_TYPE_4` (1)
- `LINK_TO_TEMPLATE` (1)
- `LINK_TO_FORMS` (1)
- `LINK_TO_PORTAL` (1)
- `LINK_TO_FLOORPLAN` (1)
- `LINK_TO_STEPS` (1)
- `LINK_TO_GUIDELINES` (1)
- `LINK_TO_MOU` (1)
- `LINK_TO_CHECKLIST` (1)
- `LINK_TO_SAMPLES` (1)
- `BRIEF_MISSION_DESCRIPTION` (1)
- `LIST_DEPARTMENTS` (1)
- `ENTRY_LEVEL_ROLES` (1)
- `LEADERSHIP_ROLES` (1)
- `GOVERNANCE_STRUCTURE` (1)
- `LIST_POLICY_DOCUMENTS` (1)
- `APPROVAL_PROCESS` (1)
- `EXCEPTION_APPROVERS` (1)
- `LINK_TO_HR_PORTAL` (1)
- `LINK_TO_ANNUAL_CALENDAR` (1)
- `LINK_TO_COVERAGE_PLANNING_TEMPLATE` (1)
- `LINK_TO_FORM` (1)
- `LINK_TO_PURCHASE_REQUEST_FORM` (1)
- `LINK_TO_EXPENSE_REPORT_TEMPLATE` (1)
- `LINK_TO_APPROVED_VENDOR_LIST` (1)
- `LINK_TO_PERFORMANCE_DOCUMENTATION_FORM` (1)
- `LINK_TO_COACHING_CONVERSATION_GUIDE` (1)
- `POLICY_DOCUMENT` (1)
- `PRIMARY_IMPACT` (1)
- `BRIEF_DESCRIPTION` (1)
- `MINIMUM_AGE` (1)
- `TRAINING_REQUIREMENTS` (1)
- `REQUIRED_SKILLS` (1)
- `BENEFIT_1` (1)
- `BENEFIT_2` (1)
- `BENEFIT_3` (1)
- `POLICY_1` (1)
- `BRIEF_EXPLANATION` (1)
- `POLICY_2` (1)
- `POLICY_3` (1)
- `BRIEF_MISSION_STATEMENT` (1)
- `APPLICATION_LINK` (1)
- `MINIMUM_COMMITMENT` (1)
- `CANCELLATION_NOTICE_PERIOD` (1)
- `RESCHEDULING_METHOD` (1)
- `SUBSTITUTE_POLICY` (1)
- `PORTAL_LINK` (1)
- `NOTIFICATION_METHOD` (1)
- `HANDBOOK_LINK` (1)
- `VIDEO_LINK` (1)
- `ELIGIBILITY_OVERVIEW` (1)
- `KEY_PARTNERS` (1)
- `CAPACITY_CHALLENGES` (1)
- `CULTURAL_FRAMEWORK` (1)
- `TRAUMA_APPROACH` (1)
- `COMMON_BARRIERS` (1)
- `SUPPORT_PROTOCOLS` (1)
- `RESOURCE_DATABASE` (1)
- `RESOURCE_LINK` (1)
- `LOCAL_NONPROFIT_CENTER` (1)
- `ENVIRONMENTAL_EDUCATION_NETWORK` (1)
- `LOCAL_COMMUNITY_FOUNDATION` (1)
- `CORPORATE_PARTNER` (1)
- `FEDERAL_AGENCY` (1)
- `HEALTHCARE_ASSOCIATION` (1)
- `HEALTH_FOUNDATION` (1)
- `HEALTHCARE_CORPORATION` (1)
- `KEY_RESOURCES` (1)
- `CHALLENGE_1` (1)
- `CHALLENGE_2` (1)
- `CHALLENGE_3` (1)
- `INTERNAL_RESOURCE_1` (1)
- `INTERNAL_RESOURCE_2` (1)
- `INTERNAL_RESOURCE_3` (1)
- `ESCALATION_TRIGGER_1` (1)
- `ESCALATION_TRIGGER_2` (1)
- `ESCALATION_TRIGGER_3` (1)
- `SUPERVISOR_ROLE` (1)
- `ESCALATION_PROCEDURE` (1)
- `ISSUE_DESCRIPTION` (1)
- `TALENT_CHALLENGES` (1)
- `SKILL_AREA` (1)
- `CURRENT_SKILL_LEVEL` (1)
- `CAREER_GOALS` (1)
- `TITLE` (1)
- `MONTH` (1)
- `AVAILABLE_POSITIONS` (1)
- `APPOINTMENT_TYPE` (1)
- `CALENDLY_LINK` (1)
- `RELEVANT_DOCUMENT` (1)
- `DURATION` (1)
- `DISCUSSION_TOPIC` (1)
- `PROMISED_RESOURCES` (1)
- `ACTION_ITEMS` (1)
- `LOCATION_DETAILS` (1)
- `PREPARATION_STEPS` (1)
- `DOCUMENT_LIST` (1)
- `PRIORITY_AGENDA_ITEMS` (1)
- `DAYS_IN_ADVANCE` (1)
- `MATERIALS_DAYS_ADVANCE` (1)
- `REMINDER_DAYS_ADVANCE` (1)
- `FOLLOW_UP_DAYS` (1)
- `KEY_AGENDA_ITEMS` (1)
- `DAY` (1)
- `CRISIS_LINE` (1)
- `STARTING_SKILL_LEVEL` (1)
- `LINKEDIN_LINK` (1)
- `KEY_DETAILS` (1)
- `RELATIONSHIP_CONTEXT` (1)
- `ACTION_TYPE` (1)
- `SMALL_GIFT_THRESHOLD` (1)
- `LARGE_GIFT_THRESHOLD` (1)
- `LOCAL_FOOD_BANK` (1)
- `DIETARY_NOTES` (1)
- `ACCESSIBILITY_NOTES` (1)
- `EDUCATION_NONPROFIT` (1)
- `BOOTH_NUMBER` (1)
- `MISSION_STATEMENT_SHORT` (1)
- `CONFERENCE_DIRECTOR` (1)
- `FOLLOW_UP_ACTION` (1)
- `PARKING_INFO` (1)
- `WHAT_TO_BRING` (1)
- `DRESS_CODE` (1)
- `MEMBERSHIP_DURATION` (1)
- `MEMBERSHIP_TIER` (1)
- `X` (1)
- `RECENT_ACCOMPLISHMENT` (1)
- `PAYMENT_OPTIONS` (1)
- `PARKING_MAP_LINK` (1)
- `REGULATORY_BODY` (1)
- `COMPLIANCE_STANDARDS` (1)
- `COMPLIANCE_STANDARD` (1)
- `STAKEHOLDER_ROLES` (1)
- `RISK_TOLERANCE_LEVEL` (1)
- `LEGAL_RESOURCES_AVAILABLE` (1)
- `INSURANCE_REQUIREMENTS` (1)
- `INDEMNIFICATION_LANGUAGE` (1)
- `PAYMENT_TERMS_GUIDELINES` (1)
- `CONFIDENTIALITY_REQUIREMENTS` (1)
- `IP_STANDARDS` (1)
- `TERMINATION_PREFERENCES` (1)
- `APPROVAL_AUTHORITY` (1)
- `SIGNATURE_AUTHORITY` (1)
- `REVIEW_CRITERIA` (1)
- `SPECIFIC_POLICIES` (1)
- `SPECIFIC_REGULATIONS` (1)
- `VERIFICATION_CHECKLIST` (1)
- `CHECKLIST` (1)
- `POLICY_TYPE` (1)
- `CRITERION_1` (1)
- `STATUS` (1)
- `ISSUES` (1)
- `RECOMMENDATIONS` (1)
- `CRITERION_2` (1)
- `DECISION` (1)
- `SUMMARY` (1)
- `ACTIONS` (1)
- `ADDITIONAL_DOCUMENT_TYPES` (1)
- `ADDITIONAL_CRITERIA` (1)
- `LEVEL_OF_SUPERVISION` (1)
- `BACKGROUND_CHECK_CRITERIA` (1)
- `UNIQUE_IDENTIFIER_FIELDS` (1)
- `DESTINATION_SYSTEM` (1)
- `DATA_FIELDS` (1)
- `REGISTRATION_PLATFORM` (1)
- `REGISTRATION_TYPE` (1)
- `REGISTRATION_CATEGORIES` (1)
- `BANK_STATEMENTS` (1)
- `ACCOUNTING_SOFTWARE_EXPORTS` (1)
- `DONATION_PLATFORM_REPORTS` (1)
- `GRANT_EXPENDITURE_LOGS` (1)
- `VARIANCE_THRESHOLD` (1)
- `GRANT_REQUIREMENTS` (1)
- `CHART_OF_ACCOUNTS` (1)
- `PRIOR_YEAR_ACTUALS` (1)
- `SOURCE_1` (1)
- `SOURCE_2` (1)
- `DATA_SOURCE_1` (1)
- `DATA_SOURCE_2` (1)
- `SPENDING_THRESHOLDS` (1)
- `SMALL_THRESHOLD` (1)
- `APPROVAL_LEVEL_1` (1)
- `LARGE_THRESHOLD` (1)
- `APPROVAL_LEVEL_2` (1)
- `APPROVAL_LEVEL_3` (1)
- `ACCOUNTING_SYSTEM` (1)
- `PAYMENT_TERMS` (1)
- `APPROXIMATE_MEMBER_COUNT` (1)
- `MEMBERSHIP_LEVELS` (1)
- `SPECIFIC_CHALLENGES` (1)
- `MEMBERSHIP_RULES` (1)
- `DUPLICATE` (1)
- `MISSING` (1)
- `EXPIRATION_THRESHOLD` (1)
- `SEGMENTATION_CRITERIA` (1)
- `HOURLY_RATE` (1)
- `MILESTONE_HOURS` (1)
- `TRACKING_SYSTEM` (1)
- `EXPENSE_THRESHOLD` (1)
- `EXPENSE_POLICY` (1)
- `APPROVAL_THRESHOLD` (1)
- `PLATFORM_1` (1)
- `PLATFORM` (1)
- `TARGET_KEYWORDS` (1)
- `WEBSITE_SECTION` (1)
- `VIDEO_LENGTH` (1)
- `DISTRIBUTION_PLATFORM` (1)
- `AUDIENCE_TYPE` (1)
- `SECTOR` (1)
- `VALUES` (1)
- `ARTICLE_TITLE` (1)
- `MAIN_TOPIC` (1)
- `TARGET_COMPANY` (1)
- `REGISTRATION_DETAILS` (1)
- `SPECIFIC_ASK` (1)
- `ANNOUNCEMENT_TOPIC` (1)
- `ANNOUNCEMENT_TYPE` (1)
- `TARGET_AUDIENCE_LIST` (1)
- `PLATFORM_LIST` (1)
- `PRIMARY_GOAL` (1)
- `SIGNATURE` (1)
- `BRIEF_IMPACT_DESCRIPTION` (1)
- `BRIEF_SUCCESS_STORY_OR_STATISTIC` (1)
- `SIMPLE_ENGAGEMENT_OPTION` (1)
- `SENDER_TITLE` (1)
- `SPECIFIC_OUTCOME_DETAILS` (1)
- `IMPACT_POINT_1` (1)
- `IMPACT_POINT_2` (1)
- `IMPACT_POINT_3` (1)
- `RELEVANT_TESTIMONIAL_OR_STORY` (1)
- `SPECIFIC_FOLLOW_UP_PLAN` (1)
- `LEADERSHIP_OR_EXCLUSIVE_OPPORTUNITY` (1)
- `DONATION_TYPE` (1)
- `KEY_STATISTICS` (1)
- `SURVEY_DATA` (1)
- `SURVEY_TOPIC` (1)
- `PURPOSE` (1)
- `SPECIFIC_TOPIC` (1)
- `SURVEY_TYPE` (1)
- `LEARNING_TOPIC` (1)
- `SKILL_LEVEL` (1)
- `ENGAGEMENT_STYLE` (1)
- `EMERGENCY_TYPE` (1)
- `LOCATION_TYPE` (1)
- `VULNERABLE_POPULATIONS` (1)
- `SAFETY_CONCERN_TYPE` (1)
- `FACILITY_TYPE` (1)
- `FILE_LOCATION` (1)
- `DATABASE_ADMIN` (1)
- `PROCEDURE_TYPE` (1)
- `STATE` (1)
- `EPISODE_LENGTH` (1)
- `EPISODE_TOPIC` (1)
- `IMPACT_METRICS` (1)
- `TECHNICAL_SETUP` (1)
- `AUDIENCE_KNOWLEDGE` (1)
- `AUDIENCE_SIZE` (1)
- `AUDIENCE_EXPECTATIONS` (1)
- `SPEAKER_BACKGROUND` (1)
- `SPEAKING_STYLE` (1)
- `PRIMARY_OBJECTIVE` (1)
- `SUCCESS_METRICS` (1)
- `AUDIENCE` (1)
- `LENGTH_IN_MINUTES` (1)
- `VIDEO_TYPE` (1)
- `KEY_POINTS_LIST` (1)
- `YEAR` (1)
- `BENEFICIARY_STORIES` (1)
- `SPECIFIC_TOPICS` (1)
- `SWOT_ANALYSIS` (1)
- `ACHIEVEMENT_TYPE` (1)
- `SPECIAL_FORMATTING` (1)
- `CERTIFICATE_TYPE` (1)
- `NUMBER_OF_CERTIFICATES` (1)
- `ACHIEVEMENT_DETAILS` (1)
- `TEMPLATE_TYPE` (1)
- `SPECIAL_NOTES` (1)
- `RSVP_LINK` (1)
- `TICKET_COUNT` (1)
- `SURVEY_LINK` (1)
- `INVITATION_TYPE` (1)
- `TARGET_NEIGHBORHOOD` (1)
- `DISASTER_TYPE` (1)
- `GIVING_HISTORY` (1)
- `PAST_INTERACTIONS` (1)
- `TONE_AND_VOICE` (1)
- `SCHEDULING_LINK` (1)
- `KEY_BENEFITS` (1)
- `SOURCE_CONTENT` (1)
- `ACCESSIBILITY_REQUIREMENTS` (1)
- `ACCESSIBILITY_NEED` (1)
- `ACCESSIBILITY_FORMAT` (1)
- `ORIGINAL_CONTENT` (1)
- `READING_LEVEL` (1)
- `LOGO_DESCRIPTION` (1)
- `COLOR1` (1)
- `COLOR2` (1)
- `STATISTIC1` (1)
- `STATISTIC2` (1)
- `SLIDE_COUNT` (1)
- `VIEWING_CONTEXT` (1)
- `FOOD_SECURITY_NONPROFIT` (1)
- `STRATEGIC_PRIORITY` (1)
- `RESOURCE_TYPE` (1)
- `DATA_TYPE` (1)
- `SPECIFIC_OBJECTIVE` (1)
- `BENCHMARK_TYPE` (1)
- `METRICS` (1)
- `TARGET_METRICS` (1)
- `GEOGRAPHIC_LEVEL` (1)
- `YOUTH_LEARNING_ALLIANCE` (1)
- `GLOBAL_RELIEF_NETWORK` (1)
- `JURISDICTION` (1)
- `SPECIFIC_CONCERN` (1)
- `SPECIFIC_FUNCTION` (1)
- `APPENDIX` (1)
- `TECHNOLOGY_SOLUTION_TYPE` (1)
- `SPECIFIC_SOLUTION_1` (1)
- `SPECIFIC_SOLUTION_2` (1)
- `SPECIFIC_SOLUTION_3` (1)
- `PRIMARY_NEED` (1)
- `NUMBER_OF_SOLUTIONS` (1)
- `TECHNOLOGY_NEED` (1)
- `FORECAST_PERIOD` (1)
- `NUMBER_OF_SCENARIOS` (1)
- `KEY_ASSUMPTIONS` (1)
- `MISSION` (1)
- `KEY_PRIORITIES` (1)
- `GROWTH_INDICATORS` (1)
- `KEY_CHALLENGES` (1)
- `EXISTING_STRENGTHS` (1)
- `STRATEGIC_OBJECTIVES` (1)
- `CURRENT_DATA` (1)
- `TARGET_OUTCOMES` (1)
- `TARGET_REGION` (1)
- `STAKEHOLDERS` (1)
- `REGULATORY_REQUIREMENTS` (1)
- `TARGET_BENEFICIARIES` (1)
- `FOCUS_ISSUE` (1)
- `GRANT_POOL_SIZE` (1)
- `PRIORITY_AREA_1` (1)
- `PRIORITY_AREA_2` (1)
- `PRIORITY_AREA_3` (1)
- `EQUITY_FOCUS_GROUPS` (1)
- `NEXT_REVIEW_STAGE` (1)
- `LEGAL_REQUIREMENTS` (1)
- `REPORTING_EXPECTATIONS` (1)
- `MINIMUM_SCORE_THRESHOLD` (1)
- `POSITION_TITLE` (1)
- `KEY_REQUIREMENT_1` (1)
- `KEY_REQUIREMENT_2` (1)
- `MINIMUM_SCORE` (1)
- `DECISION_MAKER` (1)
- `KEY_QUALITY_1` (1)
- `KEY_QUALITY_2` (1)
- `CORE_VALUE` (1)
- `X_NUMBER` (1)
- `SCHOLARSHIP_PURPOSE` (1)
- `QUALITIES_OF_SUCCESSFUL_RECIPIENTS` (1)
- `OTHER_DECISION_FACTORS` (1)
- `SCHOLARSHIP_MISSION` (1)
- `EVALUATION_RUBRIC` (1)
- `VENDOR_PROPOSALS` (1)
- `KEY_SKILL_REQUIREMENTS` (1)
- `RETENTION_RATE` (1)
- `SPECIFIC_NEEDS` (1)
- `PRIMARY_CATEGORY_SET` (1)
- `PRIORITIZATION_CRITERIA` (1)
- `CATEGORY_1` (1)
- `CATEGORY_2` (1)
- `CATEGORY_3` (1)
- `PRIORITY_1` (1)
- `PRIORITY_2` (1)
- `PRIORITY_3` (1)
- `CAMPAIGN_PHASE` (1)
- `CURRENT_CAMPAIGN` (1)
- `STAKEHOLDER_GROUPS` (1)
- `DEPARTMENT_COUNT` (1)
- `RESPONSE_CAPACITY` (1)
- `BUSY_PERIOD_1` (1)
- `BUSY_PERIOD_2` (1)
- `SPECIAL_CONSIDERATION` (1)
- `URGENT_CATEGORY` (1)
- `HIGH_PRIORITY_CATEGORY` (1)
- `MEDIUM_PRIORITY_CATEGORY` (1)
- `LOW_PRIORITY_CATEGORY` (1)
- `AUTOMATED_RESPONSE_CATEGORY` (1)
- `DEPARTMENT_1` (1)
- `DEPARTMENT_2` (1)
- `STAKEHOLDER_TYPES` (1)
- `FEEDBACK_TYPE` (1)
- `KEY_VALUES` (1)
- `CAMPAIGN_1` (1)
- `CAMPAIGN_2` (1)
- `RECENT_NEWS` (1)
- `VOICE_CHARACTERISTICS` (1)
- `TONE_GUIDELINES` (1)
- `HASHTAG_1` (1)
- `HASHTAG_2` (1)
- `HASHTAG_3` (1)
- `ROLE_1` (1)
- `TYPE_OF_MENTIONS` (1)
- `ROLE_2` (1)
- `ROLE_3` (1)
- `BRACKETED` (1)
- `TONE_DESCRIPTION` (1)
- `NUMBER_OF_CATEGORIES` (1)
- `CATEGORIZATION_CRITERIA` (1)
- `THEME_1` (1)
- `THEME_2` (1)
- `THEME_3` (1)
- `SPECIFIC_CRITERIA` (1)
- `MATERIAL_CONTENT` (1)
- `ASSESSMENT_CRITERIA` (1)
- `SCORE` (1)
- `BRIEF_JUSTIFICATION` (1)
- `ADDITIONAL_CRITERION_1` (1)
- `ADDITIONAL_CRITERION_2` (1)
- `TOTAL_SCORE` (1)
- `MAXIMUM_POSSIBLE` (1)
- `PARTNERSHIP_TIER` (1)
- `STRENGTH_1` (1)
- `STRENGTH_2` (1)
- `STRENGTH_3` (1)
- `RECOMMENDATION_1` (1)
- `SPECIFIC_ACTION_STEP` (1)
- `RECOMMENDATION_2` (1)
- `RECOMMENDATION_3` (1)
- `BRIEF_NEXT_STEPS` (1)
- `COMMUNITY_NEED` (1)
- `PREVIOUS_OUTCOMES` (1)
- `RUBRIC_SOURCE` (1)
- `MIN_SCORE` (1)
- `MAX_SCORE` (1)
- `PASSING_THRESHOLD` (1)
- `PRIORITY_CRITERIA` (1)
- `PROPOSAL_TEXT` (1)
- `QUALITY_STANDARDS` (1)
- `MATERIAL_TITLE` (1)
- `TRAINING_MATERIAL` (1)
- `TRAINING_MATERIAL_TYPE` (1)

## Recommendations

Based on this analysis, consider implementing the following input field types:

1. **Organization Profile Fields**: Create reusable fields for organization name, mission, contact info
2. **Program/Service Fields**: Standard fields for program names, descriptions, and details
3. **Contact Information Fields**: Standardized contact fields that can be reused
4. **Date/Time Fields**: Date pickers and time inputs for scheduling
5. **Financial Fields**: Currency inputs for amounts and budgets
6. **Text Area Fields**: For longer descriptions and details

Variables used only once or twice may represent template-specific needs that require custom input fields.
